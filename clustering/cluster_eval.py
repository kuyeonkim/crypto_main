...
GRIDS = {
    "umap_hdbscan": {
        "func": umap_hdbscan,
        "space": {"n_neighbors": [i for i in range(10, 51)], "min_cluster_size": [i for i in range(5,26)]},
    },
    "tsne_dbscan": {
        "func": tsne_dbscan,
        "space": {"perplexity": [i for i in range(10, 51)], "eps": [0.1*i for i in range (2,10)]},
    },
    "gmm": {
        "func": gmm_clust,
        "space": {"n_components": [i for i in range(2,12)]},
    },
    "agglo": {
        "func": agglo_clust,
        "space": {"n_clusters": [i for i in range(2,12)], "linkage": ["ward", "average", "complete", "single"]},
    },
    "pca_kmeans": {
        "func": pca_kmeans,
        "space": {"k": [i for i in range(2,12)], "n_components": [i for i in range(2,12)]},
    },
}
...
min_cluster_num = 5
max_cluster_num = 20
def _dfs(X, keys, grids, depth, cur, func, best_sc, best_cfg, best_lbl, lo=min_cluster_num, hi=max_cluster_num):
    if depth == len(keys):
...
def grid_select(X, func, grid_dict, lo=min_cluster_num, hi=max_cluster_num):
...
def grid_all(X, func, grid_dict, lo=min_cluster_num, hi=max_cluster_num):
    keys, grids = list(grid_dict.keys()), list(grid_dict.values())
    rows = []
...
def select_cluster_model(X, min_k=5, max_k=20, n_boot=10):
    summary = {}
...
def evaluate_all(X, ids, min_k=min_cluster_num, max_k=max_cluster_num, n_boot=10):
    summary = select_cluster_model(X, min_k=min_k, max_k=max_k, n_boot=n_boot)
    stats_df = collect_stats(X)
    stats_df['elementcount'] = stats_df['sizes'].apply(lambda x: x[1] if isinstance(x, tuple) and len(x) > 1 else np.nan)
    stats_df= stats_df.sort_values(['method', 'sil'])
